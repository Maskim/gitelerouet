...
class BorderlandElatedLike {
	...
	function enqueue_scripts(){
		wp_enqueue_script( 'borderland-like', get_template_directory_uri() . '/js/eltd-like.js', 'jquery', '1.0', TRUE );
		
		wp_localize_script( 'borderland-like', 'eltdLike', array(
...
if ( ! function_exists( 'borderland_elated_create_like' ) ) {
	/*
	 * @return BorderlandElatedLike
	 */
	function borderland_elated_create_like() {
		global $eltd_like;
		
		$eltd_like = new BorderlandElatedLike();
	}
	
	add_action( 'after_setup_theme', 'borderland_elated_create_like' );
}

if ( ! function_exists( 'borderland_elated_like' ) ) {
	function borderland_elated_like() {
		echo borderland_elated_get_like();
	}
}

if ( ! function_exists( 'borderland_elated_get_like' ) ) {
	function borderland_elated_get_like() {
		global $eltd_like;
		
		return $eltd_like->add_like();
	}
}

if ( ! function_exists( 'borderland_elated_like_portfolio_list' ) ) {
	function borderland_elated_like_portfolio_list( $portfolio_project_id ) {
		global $eltd_like;
		
		return $eltd_like->add_like_portfolio_list( $portfolio_project_id );
	}
}