...
if ( ! function_exists( 'borderland_elated_seo_plugin_installed' ) ) {
	/**
	 * Function that checks if popular seo plugins are installed
	 * @return bool
	 */
	function borderland_elated_seo_plugin_installed() {
		return defined( 'WPSEO_VERSION' );
	}
}

if ( ! function_exists( 'borderland_elated_remove_yoast_json_on_ajax' ) ) {
	/**
	 ...
	 * @version 0.2
	 */
	function borderland_elated_remove_yoast_json_on_ajax( $data ) {
		
		//is current request made through ajax?
		if ( borderland_elated_is_ajax_enabled() ) {
	...
	if ( borderland_elated_seo_plugin_installed() && version_compare( WPSEO_VERSION, '1.6' ) >= 0 ) {
		add_filter( 'wpseo_json_ld_output', 'borderland_elated_remove_yoast_json_on_ajax' );
		add_filter( 'disable_wpseo_json_ld_search', 'borderland_elated_remove_yoast_json_on_ajax' );
	}
}

if ( ! function_exists( 'borderland_elated_localize_no_ajax_pages' ) ) {
	/**
	 ...
	 *
	 * @uses borderland_elated_get_objects_without_ajax()
	 * @uses borderland_elated_get_wpml_pages_for_current_page()
	 * @uses borderland_elated_get_woocommerce_pages()
	 *
	 * @version 0.1
	 */
	function borderland_elated_localize_no_ajax_pages() {
		...
		if ( borderland_elated_is_ajax_enabled() ) {
			...
			$no_ajax_pages = array_merge( $no_ajax_pages, borderland_elated_get_objects_without_ajax() );
			
			//is wpml installed?
			if ( borderland_elated_is_wpml_installed() ) {
				//get translation pages for current page and merge with main array
				$no_ajax_pages = array_merge( $no_ajax_pages, borderland_elated_get_wpml_pages_for_current_page() );
			}
			
			//is woocommerce installed?
			if ( borderland_elated_is_woocommerce_installed() ) {
				//get all woocommerce pages and products and merge with main array
				$no_ajax_pages = array_merge( $no_ajax_pages, borderland_elated_get_woocommerce_pages() );
			}
			...
			wp_localize_script( 'borderland-default-dynamic', 'no_ajax_obj', array(
				'no_ajax_pages' => $no_ajax_pages
			) );
		}
	}
	
	add_action( 'wp_enqueue_scripts', 'borderland_elated_localize_no_ajax_pages' );
}

if ( ! function_exists( 'borderland_elated_get_woocommerce_pages' ) ) {
	/**
	 ...
	 * @version 0.1
	 */
	function borderland_elated_get_woocommerce_pages() {
...
if ( ! function_exists( 'borderland_elated_get_objects_without_ajax' ) ) {
	/**
	 ...
	 * @version 0.1
	 */
	function borderland_elated_get_objects_without_ajax() {
...
if ( ! function_exists( 'borderland_elated_get_wpml_pages_for_current_page' ) ) {
	/**
	 ...
	 * @version 0.1
	 */
	function borderland_elated_get_wpml_pages_for_current_page() {
...
if ( ! function_exists( 'borderland_elated_add_ajax_meta' ) ) {
	/**
	 * Function that echoes meta data for ajax
	 */
	function borderland_elated_add_ajax_meta() {
		$global_options  = borderland_elated_return_global_options();
		
		if ( isset( $global_options['disable_eltd_seo'] ) && $global_options['disable_eltd_seo'] == 'no' && ! borderland_elated_seo_plugin_installed() ) {
			$seo_description = get_post_meta( borderland_elated_get_page_id(), "seo_description", true );
			$seo_keywords    = get_post_meta( borderland_elated_get_page_id(), "seo_keywords", true );
			?>
	...
	add_action( 'borderland_elated_action_ajax_meta', 'borderland_elated_add_ajax_meta' );
}

if ( ! function_exists( 'borderland_elated_add_header_meta' ) ) {
	/**
	 * Function that echoes meta data if our seo is enabled
	 */
	function borderland_elated_add_header_meta() {
		global $is_IE;
		$global_options  = borderland_elated_return_global_options();
		
		if ( isset( $global_options['disable_qode_seo'] ) && $global_options['disable_qode_seo'] == 'no' && ! borderland_elated_seo_plugin_installed() ) {
			$seo_description = get_post_meta( borderland_elated_get_page_id(), "seo_description", true );
			$seo_keywords    = get_post_meta( borderland_elated_get_page_id(), "seo_keywords", true );
			?>
	...
	add_action( 'borderland_elated_action_header_meta', 'borderland_elated_add_header_meta' );
}