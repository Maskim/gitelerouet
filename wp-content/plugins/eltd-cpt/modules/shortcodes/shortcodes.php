<?php
if (!function_exists('borderland_elated_register_shortcode_button')) {

    function borderland_elated_register_shortcode_button($buttons) {
...
if (!function_exists('borderland_elated_add_shortcode_functionality')) {

    function borderland_elated_add_shortcode_functionality($plugin_array) {
	    $plugin_array['eltd_shortcodes'] = ELATED_CPT_MODULES_URL_PATH . '/shortcodes/eltd_shortcodes.js';
        return $plugin_array;
    }

}

if (!function_exists('borderland_elated_shortcodes_button')) {

    function borderland_elated_shortcodes_button() {
        if (!current_user_can('edit_posts') && !current_user_can('edit_pages')) {
            return;
        }

        if (get_user_option('rich_editing') == 'true') {
            add_filter('mce_external_plugins', 'borderland_elated_add_shortcode_functionality');
            add_filter('mce_buttons', 'borderland_elated_register_shortcode_button');
        }
    }

}
add_action('after_setup_theme', 'borderland_elated_shortcodes_button');
...
if (!function_exists('no_call_to_action')) {
    ...
    function no_call_to_action($atts, $content = null) {
        ...
        $html.= '<div class="call_to_action ' . esc_attr($action_classes) . '" '.borderland_elated_get_inline_style($action_styles).'>';
        ...
        if ($show_button == 'yes') {
            if ($grid_size == "75") {
                $html .= '<div class="call_to_action_row_75_25 clearfix" '.borderland_elated_get_inline_style($action_inner_styles).'>';
            }
            elseif ($grid_size == "66") {
                $html .= '<div class="call_to_action_row_66_33 clearfix" '.borderland_elated_get_inline_style($action_inner_styles).'>';
            }
            else {
                $html .= '<div class="call_to_action_row_50_50 clearfix" '.borderland_elated_get_inline_style($action_inner_styles).'>';
            }
        ...
        if ($type == "with_icon" || $type == "with_custom_icon") {
            ...
            $html .= '<div class="call_to_action_icon_inner" '.borderland_elated_get_inline_style($icon_inner_style).'>';
            if ($custom_icon != "") {
                ...
                $html .= '<img src="' . esc_attr($custom_icon_src) . '" alt="'.esc_attr__("Custom Image","eltd-cpt").'">';
            } else {
        ...
        $html .= '<div class="call_to_action_text'. esc_attr($call_to_action_text_class) .'" '.borderland_elated_get_inline_style
        ...
        if ($show_button == 'yes') {
            ...
            $html .= '<a href="' . esc_url($button_link) . '" class="qbutton ' . esc_attr($button_size) . '" target="' . esc_attr($button_target) . '" '.borderland_elated_get_inline_style($button_styles).' ' . $data_attr . '>' . wp_kses_post($button_text) . '</a>';
    ...
    function no_blockquote($atts, $content = null) {
        ...
        if ($show_quote_icon == 'yes') {
            if($quote_icon_font == 'with_icon' && $quote_icon_pack != ''){
                ...
                $html .= $icon_collections->getQuoteIcon($quote_icon_pack, true);
                $html .= '</span>';
...
if (!function_exists('no_counter')) {
    ...
    function no_counter($atts, $content = null) {
        ...
        $html .= '<div class="eltd_counter_holder ' . $counter_holder_classes . '" style="' . $counter_holder_styles . '">';
        $html .= '<span class="counter ' . $counter_classes . '" style="' . $counter_styles . '">' . $digit . '</span>';

		if ($separator == "yes" && $separator_position == "above_title" ) {
            $html .= '<span class="separator medium" ' . borderland_elated_get_inline_style($separator_styles) . '></span>';
        }
        ...
        $html .= '</div>'; //close eltd_counter_holder
...
//		innner div style
        ...
        $html .= '<div class="custom_font_holder" '.  borderland_elated_get_inline_style($custom_font_wrapper_styles).' '.$data_attr.'>';
		$html .= '<div '.$custom_font_inner_class.' '.borderland_elated_get_inline_style($custom_font_inner_style).'>'.$content.'</div>';
...
if (!function_exists('no_cover_boxes')) {
    ...
    function no_cover_boxes($atts, $content = null) {
        ...
        $html .= "<div class='cover_boxes ".esc_attr($items_number_class)."' data-active-element='" . $active_element . "' ".borderland_elated_get_inline_attr($items_number,"date-number_of_items")."><ul class='clearfix'>";
...
if (!function_exists('no_dropcaps')) {
    ...
    function no_dropcaps($atts, $content = null) {
        ...
        $html = "<span class='eltd_dropcap " . $type . "' style='";
        if ($background_color != "") {
...
if (!function_exists('no_icons')) {
    ...
    function no_icons($atts, $content = null) {
        ...
        $default_atts = array_merge($default_atts, $icon_collections->getShortcodeParams());
        ...
        $fa_size = $icon_collections->getIconSizeClass($fa_size);
        ...
        $html = '<span class="eltd_icon_shade eltd_icon_shortcode ' . esc_attr($icon_pack . ' ' . $type . ' ' . $icon_stack_classes . ' ' . $icon_animation) . '" ' . $data_attr_stack . ' '.borderland_elated_get_inline_style($icon_stack_style.$animation_delay_style).'>';
        if ($link != "") {
            $html .= '<a href="' . esc_url($link) . '" target="' . esc_attr($target) . '" '.borderland_elated_get_inline_style($icon_link_style).' '. borderland_elated_get_class_attribute($icon_link_classes) . '>';
        }
...
if (!function_exists('no_icon_text')) {
    ...
    function no_icon_text($atts, $content = null) {
        ...
        $icon_size = $icon_collections->getIconSizeClass($icon_size);
        ...
        if ($icon_animation_delay != "" && $icon_animation == "eltd_icon_animation") {
            ...
            case 'circle':
                ...
                if (method_exists($icon_collection_obj, 'render')) {
                    ...
                    if ($icon_pack == 'font_elegant' && !empty($fe_icon)) {
                        ...
                        $html_icon .= '<span class="eltd_font_elegant_holder ' . $icon_type . ' ' . $icon_stack_classes . '" style="' . $icon_stack_style . $icon_stack_base_style . '">';
                        $html_icon .= $icon_collection_obj->render(${$icon_collection_obj->param}, array(
                            'icon_attributes' => array(
                                'style' => $icon_stack_font_size,
                                'class' => 'icon_text_icon eltd_font_elegant_icon '
                            )
                        ));
                        $html_icon .= '</span>';
                    } else {

                        $html_icon .= '<span class="eltd_icon_stack ' . $icon_size . ' ' . $icon_stack_classes . '" style="' . $icon_stack_style . $icon_stack_base_style . '">';
                        $html_icon .= $icon_collection_obj->render(${$icon_collection_obj->param}, array(
                            'icon_attributes' => array(
                                'style' => $icon_stack_font_size,
                                'class' => 'icon_text_icon eltd_icon_stack_1x'
                            )
                        ));
                        $html_icon .= '</span>';
                    }
            ...
            case 'square':
                ...
                if (method_exists($icon_collection_obj, 'render')) {
                    ...
                    if ($icon_pack == 'font_elegant' && !empty($fe_icon)) {
                        ...
                        $html_icon .= '<span class="eltd_font_elegant_holder ' . $icon_type . ' ' . $icon_stack_classes . '" style="' . $icon_stack_style . $icon_stack_square_style . '">';
                        $html_icon .= $icon_collection_obj->render(${$icon_collection_obj->param}, array(
                            'icon_attributes' => array(
                                'style' => $icon_stack_font_size,
                                'class' => 'icon_text_icon eltd_font_elegant_icon '
                        ...
                        $html_icon .= '<span class="eltd_icon_stack ' . $icon_size . ' ' . $icon_stack_classes . '" style="' . $icon_stack_style . $icon_stack_square_style . '">';
                        $html_icon .= $icon_collection_obj->render(${$icon_collection_obj->param}, array(
                            'icon_attributes' => array(
                                'style' => $icon_stack_font_size,
                                'class' => 'icon_text_icon eltd_icon_stack_1x'
            ...
            default:
                ...
                if (method_exists($icon_collection_obj, 'render')) {
                    ...
                    if ($icon_pack == 'font_elegant' && $fe_icon != '') {
                        ...
                        $html_icon .= '<span class="eltd_font_elegant_holder ' . $icon_type . ' ' . $icon_stack_classes . '" style="' . $icon_stack_style . '">';
                        $html_icon .= $icon_collection_obj->render(${$icon_collection_obj->param}, array(
                            'icon_attributes' => array(
                                'style' => $icon_stack_font_size,
                                'class' => 'icon_text_icon eltd_font_elegant_icon '
                        ...
                        $html_icon .= '<span class="eltd_icon_stack ' . $icon_size . ' ' . $icon_stack_classes . '" style="' . $icon_stack_style . '">';
                        $html_icon .= $icon_collection_obj->render(${$icon_collection_obj->param}, array(
                            'icon_attributes' => array(
                                'style' => $icon_stack_font_size,
                                'class' => 'icon_text_icon eltd_icon_stack_1x'
...
//            OPEN SHORTCODE HTML
            $html .= "<div class='eltd_icon_with_title " . $icon_with_text_clasess . "'>";
...
//            ICON POSITION LEFT
            ...
            $html .= '<div class="eltd_box_holder with_icon" style="' . $box_holder_styles . '">';
...
if (!function_exists('no_icon_list_item')) {
    ...
    function no_icon_list_item($atts, $content = null) {
        ...
        $html .= '<div class="eltd_icon_list" ' .$item_margin_style .'>';

        $html .= '<div class="eltd_icon_list_icon_holder">';
        $html .= '<div class="eltd_icon_list_icon_holder_inner">';
        ...
        $html .= '</div>'; // close eltd_icon_list_icon_holder_inner
        $html .= '</div>'; // close eltd_icon_list_icon_holder

        $html .= '<p style="' . $title_style . '">' . $title . '</p>';
        $html .= '</div>'; // close eltd_icon_list
        return $html;
    }
...
if (!function_exists('no_interactive_banners')) {
    ...
    function no_interactive_banners($atts, $content = null) {
        ...
        //generate output
        $html .= '<div class="eltd_image_with_text_over ' . $custom_classes . ' ' . $layout_width . '">';
        $html .= '<div class="shader" ' . $shader_style . ' ' . $data_attr_shader . '></div>';
        if ($link_over_content == 'yes' && $show_button == "never") {
            $html .= '<a class="eltd_image_with_text_link_class" target="_self" href="' . $link . '"></a>';
        }
...
if (!function_exists('no_image_with_text_and_icon')) {
    ...
    function no_image_with_text_and_icon($atts, $content = null) {
        ...
        foreach ($icon_collections->iconCollections as $icon_set) {
            if (${$icon_set->param}) {
        ...
        $html .= '<div class="eltd_image_with_text_and_icon">';
        ...
        $html .= '<div class="eltd_icon_holder" style="bottom:' . $icon_position . 'px;">';
        ...
        $html .= '<div style="margin-top:'.$position_top.'px;">';
            if($title != ""){
                $html .= '<'.$title_tag.' class="eltd_image_with_text_and_icon_title"';
                if($title_color != ""){
                    $html .= ' style="color:'.$title_color.';"';
                }
                $html .= '>' . $title . '</'.$title_tag.'>';
        ...
        $html .= '</div>'; // close eltd_image_with_text_and_icon
        return $html;
    }
...
if (!function_exists('no_blog_list')) {
    ...
    function no_blog_list($atts, $content = null) {
        $blog_show_comments = "";
        if (borderland_elated_options()->getOptionValue( 'blog_show_comments' )) {
            $blog_show_comments = borderland_elated_options()->getOptionValue( 'blog_show_comments' );
        }
        ...
        while ($q->have_posts()) : $q->the_post();
            ...
            switch ($type) {
				...
				case 'image_in_box':
                        ...
                        $post_info_html .= '<div class="post_info_section" ' . borderland_elated_get_inline_style($latest_post_info_style_array) . '>';
                        ...
                        if ($blog_show_comments == "yes" && $display_comments == "1") {
                          $comments_count = get_comments_number();
                            ...
                            switch ($comments_count) {
                                case 0:
                                    $comments_count_text = esc_html__('No comment', 'eltd-cpt');
                                    break;
                                case 1:
                                    $comments_count_text = $comments_count . ' ' . esc_html__('Comment', 'eltd-cpt');
                                    break;
                                default:
                                    $comments_count_text = $comments_count . ' ' . esc_html__('Comments', 'eltd-cpt');
                                    break;
                            }
                            $post_info_html .= '<div class="latest_post_comments"> ';
                            $post_info_html .= '<a '.borderland_elated_get_inline_style($latest_post_info_link_color).' class="post_comments" href="' . esc_url(get_comments_link()) . '">';
                        ...
                        if ($display_category == '1') {
                            ...
                            $post_info_html .= esc_html__('in ','eltd-cpt');

                            foreach ($cat as $categ) {
                                $post_info_html .= '<a '.borderland_elated_get_inline_style($latest_post_info_link_color).' href="' . esc_url(get_category_link($categ->term_id)) . '">' . esc_html($categ->cat_name) . ' </a> ';
                            }
                        ...
                        if ($display_author == '1') {
                            ...
                            $post_info_html .= '<span '.borderland_elated_get_inline_style($latest_post_info_color).'>' . esc_html__("by", "eltd-cpt") . '</span> <a class="post_author_link" href="' . esc_url(get_author_posts_url(get_the_author_meta("ID"))) . '"><span '.borderland_elated_get_inline_style($latest_post_info_link_color).' >' . esc_html(get_the_author_meta("display_name")) . '</span></a>';
                            $post_info_html .= '</div>'; //close span.latest_post_categories
                        }
                        ...
                        $html .= '<div class="latest_post" ' . borderland_elated_get_inline_style($box_info_style) . '>';
                        ...
                        $html .= '<' . $title_tag . ' class="latest_post_title " ' . borderland_elated_get_inline_style($title_style) . '>';
                        if ($display_date == '1') {
                            $html .= '<span class="date" ' . borderland_elated_get_inline_style($date_style) . '>' . get_the_time('d M') . ' </span>'; //close date_hour_holder
                        }
                        ...
                        if ($display_excerpt == '1' && $text_length != '0') {
                            ...
                            $html .= '<p class="excerpt" ' . borderland_elated_get_inline_style($excerpt_style) . '>' . $excerpt . '...</p>';
                        }
				...
				case 'boxes':
                        ...
                        $info_position_class = "post_info_boxes_top";
                        if ($info_position == "bottom") {
                            $info_position_class = "post_info_boxes_bottom";
                        }

                        // generate post info start into $post_info_html
                        //generate comments part of description
                        $post_info_html .= '<div class="post_info_section ' . esc_attr($info_position_class) . '" '.borderland_elated_get_inline_style($latest_post_info_style_array).'>';

                        if ($display_date == '1' && $date_place == 'by_post_info') {
                             $post_info_html .= '<span class="date" ' . borderland_elated_get_inline_style($date_style) . '>' . get_the_time('d M') . ' </span>'; //close date
                        }
                        ...
                        $html .= '<li class="clearfix" ' . borderland_elated_get_inline_style($minimal_style) . '>';

                        $html .= '<div class="box_padding_border">';

                        if ($show_thumbnail == 'yes') {
                            $html .= '<div class="boxes_image">';
                            $html .= '<a href="' . esc_url(get_permalink()) . '">' . get_the_post_thumbnail(get_the_ID(), $thumb_image_size) . '<span class="latest_post_overlay" ' . borderland_elated_get_inline_style($latest_post_overlay_style) . '>';
                            if ($overlay_icon == "1") {
                ...
                case 'minimal':
                        ...
                        $html .= '<div class="latest_post" ' . borderland_elated_get_inline_style($box_style) . '>';
...
if (!function_exists('no_line_graph')) {
    ...
    function no_line_graph($atts, $content = null) {
        ...
        $id = mt_rand(1000, 9999);
        $html = "<div class='eltd_line_graf_holder'><div class='eltd_line_graf'><canvas id='lineGraph" . $id . "' height='" . $height . "' width='" . $width . "'></canvas></div><div class='eltd_line_graf_legend'><ul>";
        ...
        for ($i = 0; $i < count($line_graph_array); $i = $i + 1) {
            ...
            $color = borderland_elated_hex2rgb(trim($line_graph_el[0]));
            $html .= "{fillColor: 'rgba(" . $color[0] . "," . $color[1] . "," . $color[2] . ",0.7)',data:[" . $values . "]}";
        }
        if (borderland_elated_options()->getOptionValue( 'text_fontsize' )) {
            $text_fontsize = borderland_elated_options()->getOptionValue( 'text_fontsize' );
        } else {
            $text_fontsize = 15;
        }
        if (borderland_elated_options()->getOptionValue( 'text_color' ) && $custom_color == "") {
            $text_color = borderland_elated_options()->getOptionValue( 'text_color' );
        } else if ((borderland_elated_options()->getOptionValue( 'text_color' ) === '' || borderland_elated_options()->getOptionValue( 'text_color' ) === false ) && $custom_color != "") {
            $text_color = $custom_color;
        } else if (borderland_elated_options()->getOptionValue( 'text_color' ) && $custom_color != "") {
            $text_color = $custom_color;
        } else {
            $text_color = '#818181';
        }
...
if (!function_exists('no_message')) {
    ...
    function no_message($atts, $content = null) {
        ...
        $html .= "<div class='eltd_message " . $message_classes . "' style='" . $message_styles . "'>";
        $html .= "<div class='eltd_message_inner'>";
        if ($type == "with_icon") {
            $icon_html .= '<div class="eltd_message_icon_holder ' . $icon_position . '"><div class="eltd_message_icon"><div class="eltd_message_icon_inner">';
            if ($custom_icon != "") {
                if (is_numeric($custom_icon)) {
                    $custom_icon_src = wp_get_attachment_url($custom_icon);
                } else {
                ...
                $icon_html .= '<img src="' . $custom_icon_src . '" alt="'.esc_attr__("Custom Image","eltd-cpt").'">';
            } else {
                $icon_collection_obj = $icon_collections->getIconCollection($icon_pack);
        ...
        $html .= "<i class='eltd_font_elegant_icon icon_close' style='" . $close_button_style . "'></i>";
...
if (!function_exists('no_pie_chart')) {
    ...
    function no_pie_chart($atts, $content = null) {
        ...
        $html .= '<div class="eltd_pie_chart_holder"><div class="eltd_percentage" data-alignment="' . $chart_alignment . '" data-percent="' . $percent . '" data-linewidth="' . $line_width . '" data-size="' . $size . '" data-active="' . $active_color . '" data-noactive="' . $noactive_color . '"';
...
if (!function_exists('no_pie_chart_with_icon')) {
    function no_pie_chart_with_icon($atts, $content = null) {
        ...
        $html .= '<div class="eltd_pie_chart_with_icon_holder"><div class="eltd_percentage_with_icon" data-percent="' . $percent . '" data-linewidth="' . $line_width . '"  data-size="' . $size . '" data-active="' . $active_color . '" data-noactive="' . $noactive_color . '">';
...
if (!function_exists('no_pie_chart2')) {
    ...
    function no_pie_chart2($atts, $content = null) {
        ...
        $id = mt_rand(1000, 9999);
        $html = "<div class='eltd_pie_graf_holder'><div class='eltd_pie_graf'><canvas id='pie" . $id . "' height='" . $height . "' width='" . $width . "'></canvas></div><div class='eltd_pie_graf_legend'><ul>";
...
if (!function_exists('no_progress_bar')) {
    ...
    function no_progress_bar($atts, $content = null) {
        ...
        $html .= "<div class='eltd_progress_bar'>";
...
if (!function_exists('no_progress_bar_vertical')) {
    ...
    function no_progress_bar_vertical($atts, $content = null) {
        ...
        $html .= "<div class='eltd_progress_bars_vertical'>";
...
if (!function_exists('no_progress_bar_icon')) {
    ...
    function no_progress_bar_icon($atts, $content = null) {
        ...
        $html = "<div class='eltd_progress_bars_icons_holder'><div class='eltd_progress_bars_icons'><div class='eltd_progress_bars_icons_inner " . $type . " " . $size;

        $html .= " clearfix' data-number='" . $active_number . "'>";

        $i = 0;
        while ($i < $icons_number) {
            $html .= "<div class='bar' ".borderland_elated_get_inline_style($custom)."><span class='bar_noactive eltd_icon_stack ";
            if ($size != "") {
                if ($size == "tiny") {
                    $html .= "eltd_tiny_icon";
                } else if ($size == "small") {
                    $html .= "eltd_small_icon";
                } else if ($size == "medium") {
                    $html .= "eltd_medium_icon";
                } else if ($size == "large") {
                    $html .= "eltd_large_icon";
            ...
            if (method_exists($icon_collection_obj, 'render')) {
                ...
                $html .= $icon_collection_obj->render(${$icon_collection_obj->param}, array(
                    'icon_attributes' => array(
                        ...
                        'class' => 'eltd_icon_stack_1x'
                    )
                ));

            }

            $html .= "</span><span class='bar_active eltd_icon_stack ";
            if ($size != "") {
                if ($size == "tiny") {
                    $html .= "eltd_tiny_icon";
                } else if ($size == "small") {
...
if (!function_exists('no_social_icons')) {
    ...
    function no_social_icons($atts, $content = null) {
        ...
        $html .= "<span class='eltd_social_icon_holder " . implode(' ', $icon_holder_classes) . "' $data_attr>";
        ...
        if ($type == "normal_social") {
            ...
            if (method_exists($icon_collection_obj, 'render')) {
                ...
                $html .= "<span class='eltd_icon_stack " . $size . " " . $type . "' style='" . $icon_styles . $fa_stack_styles . "'>";
        ...
        $html .= "</span>"; //close eltd_social_icon_holder
        return $html;
    }
...
if (!function_exists('no_social_share')) {
    ...
    function no_social_share($atts, $content = null) {
        if (borderland_elated_options()->getOptionValue( 'twitter_via' )) {
            $twitter_via = esc_html__( " via ", 'eltd-cpt' ) . esc_attr(borderland_elated_options()->getOptionValue( 'twitter_via' )) . " ";
        } else {
            $twitter_via = "";
        }
        ...
        if (borderland_elated_options()->getOptionValue( 'enable_social_share' ) == "yes") {
            ...
            if (borderland_elated_options()->getOptionValue( "post_types_names_$post_type" )) {
                if (borderland_elated_options()->getOptionValue( "post_types_names_$post_type" ) == $post_type) {
                    ...
                    $html .= '<span class="social_share_title">' . esc_html__('Share', 'eltd-cpt') . '</span>';
                    $html .= '</a>';
                    $html .= '<div class="social_share_dropdown"><ul>';
                    if (borderland_elated_options()->getOptionValue( 'enable_facebook_share' ) == "yes") {
                        $html .= '<li class="facebook_share">';
                        if(wp_is_mobile()) {
                            $html .= '<a title="'.esc_attr__("Share on Facebook","eltd-cpt").'" href="javascript:void(0)" onclick="window.open(\'https://m.facebook.com/sharer.php?u=' . urlencode(get_permalink());
                        }
                        else {
                            $html .= '<a title="'.esc_attr__("Share on Facebook","eltd-cpt").'" href="javascript:void(0)" onclick="window.open(\'https://www.facebook.com/sharer.php?s=100&amp;p[title]=' . urlencode(esc_attr(get_the_title())) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;p[images][0]=';
                        ...
                        if (borderland_elated_options()->getOptionValue( 'facebook_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( "facebook_icon" )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
                    ...
                    if (borderland_elated_options()->getOptionValue( 'enable_twitter_share' ) == "yes") {
                        $html .= '<li class="twitter_share">';
                        if(wp_is_mobile()) {
                            $html .= '<a href="#" onclick="popUp=window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(borderland_elated_the_excerpt_max_charlength() . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                        }
                        else {
                            $html .= '<a href="#" onclick="popUp=window.open(\'https://twitter.com/home?status=' . urlencode(borderland_elated_the_excerpt_max_charlength() . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                        }
                        if (borderland_elated_options()->getOptionValue( 'twitter_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( "twitter_icon" )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
                    ...
                    if (borderland_elated_options()->getOptionValue( 'enable_google_plus' ) == "yes") {
                        ...
                        if (borderland_elated_options()->getOptionValue( 'google_plus_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( 'google_plus_icon' )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
                    ...
                    if (borderland_elated_options()->getOptionValue( 'enable_linkedin' ) == "yes") {
                        ...
                        if (borderland_elated_options()->getOptionValue( 'linkedin_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( 'linkedin_icon' )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
                    ...
                    if (borderland_elated_options()->getOptionValue( 'enable_tumblr' ) == "yes") {
                        ...
                        if (borderland_elated_options()->getOptionValue( 'tumblr_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( 'tumblr_icon' )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
                    ...
                    if (borderland_elated_options()->getOptionValue( 'enable_pinterest' ) == "yes") {
                        ...
                        if (borderland_elated_options()->getOptionValue( 'pinterest_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( 'pinterest_icon' )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
                    ...
                    if (borderland_elated_options()->getOptionValue( 'enable_vk' ) == "yes") {
                        ...
                        if (borderland_elated_options()->getOptionValue( 'vk_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( 'vk_icon' )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
...
if (!function_exists('no_social_share_list')) {
    ...
    function no_social_share_list($atts, $content = null) {
        ...
        if (borderland_elated_options()->getOptionValue( 'enable_social_share' ) == "yes") {
            ...
            $share_icons_type = '';
            if(borderland_elated_options()->getOptionValue( 'social_share_list_icons_type' ) !== '') {
                $share_icons_type = borderland_elated_options()->getOptionValue( 'social_share_list_icons_type' );
            }
            ...
            if (borderland_elated_options()->getOptionValue( "post_types_names_$post_type" )) {
                if (borderland_elated_options()->getOptionValue( "post_types_names_$post_type" ) == $post_type) {
                    ...
                    if (borderland_elated_options()->getOptionValue( 'enable_twitter_share' ) == "yes") {
                        $html .= '<li class="twitter_share">';
                        if(wp_is_mobile()) {
                            $html .= '<a href="#" title="'.esc_attr__("Share on Twitter", 'eltd-cpt').'" onclick="popUp=window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(borderland_elated_the_excerpt_max_charlength(mb_strlen(get_permalink())) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                        }
                        else {
                            $html .= '<a href="#" title="'.esc_attr__("Share on Twitter", 'eltd-cpt').'" onclick="popUp=window.open(\'https://twitter.com/home?status=' . urlencode(borderland_elated_the_excerpt_max_charlength(mb_strlen(get_permalink())) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                        }
                        if (borderland_elated_options()->getOptionValue( 'twitter_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( "twitter_icon" )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
                    ...
                    if (borderland_elated_options()->getOptionValue( 'enable_google_plus' ) == "yes") {
                        $html .= '<li  class="google_share">';
                        $html .= '<a href="#" title="' . esc_attr__("Share on Google+", "eltd-cpt") . '" onclick="popUp=window.open(\'https://plus.google.com/share?url=' . urlencode(get_permalink()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (borderland_elated_options()->getOptionValue( 'google_plus_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( 'google_plus_icon' )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
                    ...
                    if (borderland_elated_options()->getOptionValue( 'enable_linkedin' ) == "yes") {
                        $html .= '<li  class="linkedin_share">';
                        $html .= '<a href="#" class="' . esc_attr__("Share on LinkedIn", "eltd-cpt") . '" onclick="popUp=window.open(\'https://linkedin.com/shareArticle?mini=true&amp;url=' . urlencode(get_permalink()) . '&amp;title=' . urlencode(get_the_title()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (borderland_elated_options()->getOptionValue( 'linkedin_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( 'linkedin_icon' )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
                    ...
                    if (borderland_elated_options()->getOptionValue( 'enable_tumblr' ) == "yes") {
                        $html .= '<li  class="tumblr_share">';
                        $html .= '<a href="#" title="' . esc_attr__("Share on Tumblr", "eltd-cpt") . '" onclick="popUp=window.open(\'https://www.tumblr.com/share/link?url=' . urlencode(get_permalink()) . '&amp;name=' . urlencode(get_the_title()) . '&amp;description=' . urlencode(get_the_excerpt()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (borderland_elated_options()->getOptionValue( 'tumblr_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( 'tumblr_icon' )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
                    ...
                    if (borderland_elated_options()->getOptionValue( 'enable_pinterest' ) == "yes") {
                        $html .= '<li  class="pinterest_share">';
                        $image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full');
                        $html .= '<a href="#" title="' . esc_attr__("Share on Pinterest", "eltd-cpt") . '" onclick="popUp=window.open(\'https://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . esc_attr(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (borderland_elated_options()->getOptionValue( 'pinterest_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( 'pinterest_icon' )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
                    ...
                    if (borderland_elated_options()->getOptionValue( 'enable_vk' ) == "yes") {
                        $html .= '<li  class="vk_share">';
                        $image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full');
                        $html .= '<a href="#" title="' . esc_attr__("Share on VK", "eltd-cpt") . '" onclick="popUp=window.open(\'https://vkontakte.ru/share.php?url=' . urlencode(get_permalink()) . '&amp;title=' . urlencode(get_the_title()) . '&amp;description=' . urlencode(get_the_excerpt()) . '&amp;image=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        if (borderland_elated_options()->getOptionValue( 'vk_icon' )) {
                            $html .= '<img src="' . esc_url(borderland_elated_options()->getOptionValue( 'vk_icon' )) . '" alt="'.esc_attr__("Social Share Image","eltd-cpt").'" />';
                        } else {
...
if (!function_exists('no_team')) {
    ...
    function no_team($atts, $content = null) {
        ...
        $eltd_team_holder_classes = array();

		if ($team_type != "") {
			$eltd_team_holder_classes[] = $team_type;
		}
        if ($background_color != "" || $box_border == "yes") {
            $eltd_team_holder_classes[] = "with_padding";
        }

		if ($team_social_style != "") {
			$eltd_team_holder_classes[] = $team_social_style;
		}

        $eltd_team_style = "";
        if ($background_color != "") {
            $eltd_team_style .= " style='";
            $eltd_team_style .= 'background-color:' . $background_color . ';';
            $eltd_team_style .= "'";
        }

        $eltd_team_image_hover_style = "";
        if ($team_image_hover_color != "") {
            $eltd_team_image_hover_style .= " style='";
            $eltd_team_image_hover_style .= 'background-color:' . $team_image_hover_color . ';';
            $eltd_team_image_hover_style .= "'";
        }
        ...
        $eltd_team_separator_style = '';
        if ($team_separator_color != '') {
            $eltd_team_separator_style = 'border-color: ' . $team_separator_color;
        }

        $eltd_team_name_style_array = array();
        $eltd_team_name_style = '';

        if ($team_name_color != '') {
            $eltd_team_name_style_array[] = 'color: ' . $team_name_color;
        }

        if ($team_name_font_size != '') {
            $eltd_team_name_style_array[] = 'font-size: ' . $team_name_font_size . 'px';
        }

        if ($team_name_font_weight != '') {
            $eltd_team_name_style_array[] = 'font-weight: ' . $team_name_font_weight;
        }

        if ($team_name_text_transform != '') {
            $eltd_team_name_style_array[] = 'text-transform: ' . $team_name_text_transform;
        }

        if (is_array($eltd_team_name_style_array) && count($eltd_team_name_style_array)) {
            $eltd_team_name_style = 'style ="' . implode(';', $eltd_team_name_style_array) . '"';
        }

        $eltd_team_position_style_array = array();
        $eltd_team_position_style = '';

        if ($team_position_color != '') {
            $eltd_team_position_style_array[] = 'color: ' . $team_position_color;
        }

        if ($team_position_font_size != '') {
            $eltd_team_position_style_array[] = 'font-size: ' . $team_position_font_size . 'px';
        }

        if ($team_position_font_weight != '') {
            $eltd_team_position_style_array[] = 'font-weight: ' . $team_position_font_weight;
        }

        if ($team_position_text_transform != '') {
            $eltd_team_position_style_array[] = 'text-transform: ' . $team_position_text_transform;
        }

        if (is_array($eltd_team_position_style_array) && count($eltd_team_position_style_array)) {
            $eltd_team_position_style = 'style ="' . implode(';', $eltd_team_position_style_array) . '"';
        }

        $eltd_team_description_style_array = array();
        $eltd_team_description_style = '';

        if ($team_description_color != '') {
            $eltd_team_description_style_array[] = 'color: ' . $team_description_color;
        }

        if (is_array($eltd_team_description_style_array) && count($eltd_team_description_style_array)) {
            $eltd_team_description_style = 'style ="' . implode(';', $eltd_team_description_style_array) . '"';
        }
		...
		switch ($team_type) {
			case "on_hover":
				$html = "<div class='eltd_team " . implode(' ', $eltd_team_holder_classes) . "'" . $eltd_team_style . ">";
				$html .= "<div class='eltd_team_inner'>";
				if ($team_image != "") {
					$html .= "<div class='eltd_team_image'>";
					$html .= "<img src='$team_image_src' alt='team_image' />";
					$html .= "<div class='eltd_team_social_holder " .$team_hover_type. "' " . $eltd_team_image_hover_style . ">";
					$html .= "<div class='eltd_team_social'>";
					$html .= "<div class='eltd_team_social_inner'>";


					if ($team_name !== '' || $team_position !== '' || $team_show_separator == 'yes') {
						$html .= "<div class='eltd_team_title_holder'>";
						if ($team_name !== '') {
							$html .= "<$team_name_tag class='eltd_team_name' " . $eltd_team_name_style . ">";
							$html .= $team_name;
							$html .= "</$team_name_tag>";
						}

						if ($team_show_separator == "yes") {
							$html .= "<span class='separator small' style='" . $eltd_team_separator_style . "'></span>";
						}


						if ($team_position != "") {
							$html .= "<h6 class='eltd_team_position' " . $eltd_team_position_style . ">" . $team_position . "</h6>";
						}
						$html .= "</div>"; //close div.eltd_team_title_holder
					...
					if ($team_social_icon_pack !== '') {
						$icon_collection_obj = $icon_collections->getIconCollection($team_social_icon_pack);
					...
					if ($team_social_icon_pack !== '') {
						...
						$html .= "<div class='eltd_team_social_wrapp'>";
						...
						$html .= '</div>';    //close eltd_team_social_wrapp
					}

					$html .= "</div></div></div>"; //close div.eltd_team_social_holder

					$html .= "</div>"; //close div.eltd_team_image
				}

				if ($team_description != "" || $show_skills == 'yes') {

					$html .= "<div class='eltd_team_text " . $text_align . "' " . $qteam_box_style . ">";
					$html .= "<div class='eltd_team_text_inner'>";


					if ($team_description != "") {

						$html .= "<div class='eltd_team_description'>";
						$html .= "<p " . $eltd_team_description_style . ">" . $team_description . "</p>";
						$html .= "</div>"; // close div.eltd_team_description
					}

					if ($show_skills == 'yes') {
						$html .= '<div class="eltd_team_skills_holder">';
					...
					$html .= "</div>"; //close div.eltd_team_text_inners
					$html .= "</div>"; //close div.eltd_team_text
				}
				$html .= "</div>"; //close div.eltd_team_inner
				$html .= "</div>"; //close div.eltd_team
				break;
			case "below_image":
				$html = "<div class='eltd_team  $text_align " . implode(' ', $eltd_team_holder_classes) . "'" . $eltd_team_style . ">";
				$html .= "<div class='eltd_team_inner'>";
				if ($team_image != "") {
					$html .= "<div class='eltd_team_image'>";
					$html .= "<img src='$team_image_src' alt='team_image' />";
					$html .= "<div class='image_overlay' " . $eltd_team_image_hover_style . "></div>";
					//generate social icons html
					$team_social_icon_type_label = ''; //used in generating shortcode parameters based on icon pack
					$team_social_icon_param_label = ''; //used in generating shortcode parameters based on icon pack
					...
					if ($team_social_icon_pack !== '') {
						...
						$html .= "<div class='eltd_team_social_holder_between " .$team_hover_type. " " .$social_icons_position. "'>";
						$html .= "<div class='eltd_team_social ".$team_social_icon_type." '>";
						...
						$html .= '</div>';//close eltd_team_social_wrapp
						$html .= "</div></div></div>"; //close div.eltd_team_social_holder_between
				...
				$html .= '<div class="eltd_team_info"  '. $qteam_box_style . '>';

				if ($team_name !== '' || $team_position !== '' || $team_show_separator == 'yes') {
					$html .= "<div class='eltd_team_title_holder ".$team_social_icon_type."'>";
					if ($team_name !== '') {
						$html .= "<$team_name_tag class='eltd_team_name' " . $eltd_team_name_style . ">";
						$html .= $team_name;
						$html .= "</$team_name_tag>";
				...
				$html .= "</div>"; //close div.eltd_team_info
				$html .= "</div>"; //close div.eltd_team_inner
				$html .= "</div>"; //close div.eltd_team
...
if (!function_exists('no_unordered_list')) {
    ...
    function no_unordered_list($atts, $content = null) {
        ...
        $html = "<div class='eltd_list $list_item_classes";
        if ($animate == "yes") {
...
if (!function_exists('no_service_table')) {
    ...
    function no_service_table($atts, $content = null) {
        ...
        if ($header_image != "") {
            ...
            $service_header_image .= "<img src=" . $background_image_src . " />";
        }
...
/* Elated Image Slider with no space shortcode */
...
if (!function_exists('no_image_slider_no_space')) {
    ...
    function no_image_slider_no_space($atts, $content = null) {
        ...
        $html .= "<div class='eltd_image_gallery_no_space " . $image_gallery_holder_classes . "'><div class='eltd_image_gallery_holder' style='" . $image_gallery_holder_styles . "'><ul " . $highlight_inactive_color_style . ">";
        ...
        if (isset($images_gallery_array) && count($images_gallery_array) != 0) {
            ...
            foreach ($images_gallery_array as $gimg_id) {
                ...
                if ($on_click !== '') {
                    switch ($on_click) {
                        case 'prettyphoto':
                            $html .= '<a class="lightbox_single_portfolio" rel="' . $pretty_photo_rel . '" href="' . $image_src . '">';
                            break;
        ...
        if (borderland_elated_options()->getOptionValue( 'carousel_navigation_arrows_type' ) != '') {
            $icon_navigation_class = borderland_elated_options()->getOptionValue( 'carousel_navigation_arrows_type' );
        }
...
if (!function_exists('no_google_map')) {
    ...
    function no_google_map($atts, $content = null) {
        ...
        $html .= "<div class='google_map_holder' style='height:" . $map_height . "'><div class='eltd_google_map' id='" . $holder_id . "' " . $data_attribute . "></div>";
...
if (!function_exists('no_separator_with_icon')) {
    ...
    function no_separator_with_icon($atts, $content = null) {
        ...
        $separator_classes = "eltd_separator_with_icon ";
        $separator_styles = array();
        ...
        $icon_position = '50';
        if ($type == 'with_custom_icon' && $custom_icon != '') {
            $custom_icon_dimensions_array = borderland_elated_get_image_dimensions($custom_icon_src);
        ...
        $html .= '<div class="' . esc_attr($separator_classes) . '" '.borderland_elated_get_inline_style($separator_styles).'>';
        $html .= '<div class="eltd_icon_holder" style="bottom:' . esc_attr($icon_position) . 'px;">';

        $html .= '<div class="eltd_separator_icon_holder">';

        if ($separator_icon_position == 'right' || $separator_icon_position == 'center') {
            $html .= '<div class="eltd_line_before"  style="' . esc_attr(implode(';', $separator_lines_styles) . ';' . $icon_right_margin) . '"></div>';
        }
        ...
        elseif($type == 'with_custom_icon'){
            $html .= '<div class="separator_custom_icon"><a href="'.esc_url($link).'" target="'.esc_attr($target).'" '.$icon_link_classes.'><img src="' . esc_url($custom_icon_src) . '" alt="'.esc_attr__("Custom Image","eltd-cpt").'" /></a></div>';
        }

        if ($separator_icon_position == 'left' || $separator_icon_position == 'center') {
            $html .= '<div class="eltd_line_after"  style="' . esc_attr(implode(';', $separator_lines_styles) . ';' . $icon_left_margin) . '"></div>';
        }

        $html .= '</div>'; //close separator_icon_holder

        $html .= '</div>'; //close eltd_icon_holder
...
if (!function_exists('no_blog_slider')) {
    function no_blog_slider($atts, $content = null) {
        ...
        if($query_results->have_posts()):
	        ...
	        while ( $query_results->have_posts() ) : $query_results->the_post();
                ...
                switch($type) {
                    case 'info_in_bottom':
                        ...
                        $html .= '<div class="blog_text_holder blog_slider_info_in_bottom" ' . borderland_elated_get_inline_style
                            ($hover_box_style) . '>';
                        ...
                        if ($show_date == 'yes' || $show_author=='yes' || $show_categories == 'yes') {
                            ...
                            $html.= '<div class = "blog_slider_post_info"' . borderland_elated_get_inline_style($post_info_style) . '>';
                            ...
                            if ($show_author == 'yes') {
                                $html .= '<span class="blog_slider_author_holder">' . esc_html__("by ", "eltd-cpt") . '<a href="' . get_author_posts_url(get_the_author_meta("ID")) . '" ' . borderland_elated_get_inline_style($author_style) . '>' . get_the_author_meta("display_name") . '</a></span>';
                            }

                            if ($show_categories == 'yes') {
                                $html .= '<div class="blog_slider_categories">' . esc_html__("in ", "eltd-cpt");
                    ...
                    default:
                        ...
                        $html .= '<div class="blog_text_holder blog_slider_default ' . $info_type_class . '" ' . borderland_elated_get_inline_style($hover_box_style) . '>';
                        $html .= '<div class="blog_text_holder_outer">';
            ...
            $html .= esc_html__('Sorry, no posts matched your criteria.', 'eltd-cpt');
        endif;
