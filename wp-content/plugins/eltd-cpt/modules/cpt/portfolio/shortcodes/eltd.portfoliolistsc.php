...
class EltdPortfolioListShortcode {
    ...
    public function render($atts, $content = null) {
        ...
        if (borderland_elated_options()->getOptionValue( 'portfolio_eltd_like')) {
            $portfolio_eltd_like = borderland_elated_options()->getOptionValue( 'portfolio_eltd_like');
        }

        $portfolio_filter_class = "";
        if (borderland_elated_options()->getOptionValue( 'portfolio_filter_disable_separator')) {
            if (borderland_elated_options()->getOptionValue( 'portfolio_filter_disable_separator') == "yes") {
        ...
        if ($overlay_background_color != '' || $gradient_position != '') {
            if ($hover_type == "gradient_hover") {
                if (substr($overlay_background_color, 0, 3) === "rgba") { // if rgba is set, portfolio uses default color
                    ...
                    $rgb = borderland_elated_hex2rgb($overlay_background_color);
        ...
        if ($type != 'masonry') {
            ...
            if ($filter == "yes") {
                ...
                if ($disable_filter_title != "yes") {
                    $html .= "<li class='filter_title'><span>" . esc_html__('Sort Portfolio:', 'eltd-cpt') . "</span></li>";
                }
                if ($type == 'masonry_with_space' || $type == 'masonry_with_space_without_description') {
                    $html .= "<li class='filter' data-filter='*'><span>" . esc_html__('All', 'eltd-cpt') . "</span></li>";
                } else {
                    $html .= "<li class='filter' data-filter='all'><span>" . esc_html__('All', 'eltd-cpt') . "</span></li>";
                }
	        ...
	        if ( $query_loop->have_posts() ): while ( $query_loop->have_posts() ) : $query_loop->the_post();
                    ...
                    if ($overlay_background_color != '' || $gradient_position != '') {
                        if ($hover_type == "gradient_hover") {
                            if (substr($overlay_background_color_custom, 0, 3) === "rgba") { // if rgba is set, portfolio uses default color
                                ...
                                $rgb = borderland_elated_hex2rgb($overlay_background_color_custom);
                ...
                if ($type == "standard" || $type == "standard_no_space" || $type == "masonry_with_space") {
                    $html .= "<div class='portfolio_description " . $portfolio_description_class . "' ".borderland_elated_get_inline_style(implode('; ', $portfolio_box_style)).">";
            ...
            endwhile;
                ...
                ?>
                <p><?php esc_html_e('Sorry, no posts matched your criteria.', 'eltd-cpt'); ?></p>
            <?php
            endif;
            ...
            if (get_next_posts_link(null, $query_loop->max_num_pages)) {
                if ($show_load_more == "yes" || $show_load_more == "") {
                    $html .= '<div class="portfolio_paging" ' . $show_more_style . '><span data-rel="' . $query_loop->max_num_pages . '" class="load_more">' . get_next_posts_link(esc_html__('Show more', 'eltd-cpt'), $query_loop->max_num_pages) . '</span></div>';
                    $html .= '<div class="portfolio_paging_loading"><a href="javascript: void(0)" class="qbutton">' . esc_html__('Loading...', 'eltd-cpt') . '</a></div>';
                }
            }
            ...
            vc_map( array(
                ...
                "params" => array(
                    ...
                    array(
                        ...
                        "value" => "",
                        "description" =>  esc_html__("Please insert top margin for load more button", 'eltd-cpt'),
