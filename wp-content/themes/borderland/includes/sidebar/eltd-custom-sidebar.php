 ...
 * 
 */
if( ! class_exists( 'BorderlandElatedCustomSidebar' ) ){
	class BorderlandElatedCustomSidebar{
		var $sidebars  = array();
		var $stored    = "";
		...
		function __construct(){
			...
			$this->title = esc_html__('Custom Widget Area','borderland');
		    
			add_action('load-widgets.php', array(&$this, 'load_assets') , 5 );
			add_action('widgets_init', array(&$this, 'register_custom_sidebars') , 1000 );
			add_action('wp_ajax_borderland_elated_action_delete_custom_sidebar', array(&$this, 'delete_sidebar_area') , 1000 );
		}
		...
		function load_assets(){
			...
			wp_enqueue_script('borderland-sidebar' , BORDERLAND_ROOT . '/framework/admin/assets/js/eltdf-sidebar.js');
			wp_enqueue_style( 'borderland-sidebar' , BORDERLAND_ROOT . '/framework/admin/assets/css/eltdf-sidebar.css');
		}
		
		//widget form template
		function template_add_widget_field(){
			$nonce =  wp_create_nonce ('eltd-delete-sidebar');
			$nonce = '<input type="hidden" name="eltd-delete-sidebar" value="'. esc_attr($nonce) .'" />';

			echo "\n<script type='text/html' id='eltd-add-widget'>";
			echo "\n  <form class='eltd-add-widget' method='POST'>";
			echo "\n  <h3>". esc_html($this->title) ."</h3>";
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_html__('Enter Name of the new Widget Area', 'borderland')."' name='eltd-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_html__('Add Widget Area', 'borderland')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'borderland_elated_filter_custom_widget_args', $args );
