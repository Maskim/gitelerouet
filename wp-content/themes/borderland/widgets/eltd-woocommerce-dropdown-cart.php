<?php
class Elated_Woocommerce_Dropdown_Cart extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'eltd-woocommerce-dropdown-cart', // Base ID
			'Elated Woocommerce Dropdown Cart', // Name
			array( 'description' => __( 'Elated Woocommerce Dropdown Cart', 'eltd' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if(isset($eltd_options['woo_drop_cart_type'])){
			if($eltd_options['woo_drop_cart_type']=='with_icon'){
				$cart_style = 'with_icon';
			}elseif($eltd_options['woo_drop_cart_type']=='with_icon_label'){
			...
			<div class="shopping_cart_inner">
				<div class="shopping_cart_header">
						...
						<?php } if($cart_style == "with_icon_label") { ?>
							<a class="header_cart" href="<?php echo esc_url($woocommerce->cart->get_cart_url()); ?>">
								<i class="icon_cart_alt"></i>
								<span class="cart_label"><?php _e('Cart','eltd') ?></span>
							</a>
						<?php } ?>
										...
										<li>
											...
											<div class="item_info_holder">
												<div class="item_left">
													...
													<span class="quantity"><?php _e('Quantity: ','eltd'); echo esc_html($cart_item['quantity']); ?>
												</div>
												<div class="item_right">
													<?php echo apply_filters( 'woocommerce_cart_item_price_html', wc_price( $product_price ), $cart_item, $cart_item_key ); ?>
													<?php echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf('<a href="%s" class="remove" title="%s">&times;</a>', esc_url( $woocommerce->cart->get_remove_url( $cart_item_key ) ), __('Remove this item', 'eltd') ), $cart_item_key ); ?>
...
add_action( 'widgets_init', create_function( '', 'register_widget( "Elated_Woocommerce_Dropdown_Cart" );' ) );
?>
<?php
add_filter('add_to_cart_fragments', 'eltd_woocommerce_header_add_to_cart_fragment');
function eltd_woocommerce_header_add_to_cart_fragment( $fragments ) {
	global $woocommerce; 
	global $eltd_options;
