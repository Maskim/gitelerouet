...
function perPageBindings() {
    ...
    $j('.eltd_slider_preloader .ajax_loader').hide();
    content = $j('.content');
    contentMinHeight();
...
function slideInNewPage(content, text, direction, direction2, animationTime, callbacks, url) {
    ...
    var newHTML = $j(text);
    var animation;
    var pageId = newHTML.find('#eltd_page_id').text();
            ...
            if (animation === "ajax_updown" || animation === "ajax_updown_fade") {
                ...
                if (animation === "ajax_updown_fade") {
					...
					content.stop().fadeTo(animationTime, 0, function () {
                        ...
                        setPageMeta(meta_data); // this function is called here since there need to be set new classes on body, before all function are called (ex. transparency class and eltd slider width)
...
function anchorAjaxScroll(hash){
    ...
    var scrollToAmount;
    if(hash !== undefined && $j('[data-eltd_id="'+hash+'"]').length > 0){
...
function onLinkClicked(me) {
    ...
    if(me.attr('href') === undefined){
        url = me.attr('value').split(eltd_root)[1];
    }else{
        url = me.attr('href').split(eltd_root)[1];
    }
...
function loadResource(url) {
    ...
    var me = $j("nav a[href='"+eltd_root+url+"']");
        ...
        success: function (text, status, request) {
            ...
            if (window.history.pushState) {
                ...
                if(typeof _gaq !== 'undefined') {
                    ...
                    _gaq.push(['_trackPageview', eltd_root+url]);
                }
            } else {
                document.location.href = eltd_root + '#/' + url;
            }
        },
        error: function () {
...
function showActivePage(){
    ...
    if ((document.location.href.indexOf("?s=") >= 0) || (document.location.href.indexOf("?animation=") >= 0) || (document.location.href.indexOf("?menu=") >= 0) || (document.location.href.indexOf("?footer=") >= 0)) {
        ...
        ajaxSetActiveState($j("nav a[href='"+eltd_root+"']"));
        return;
    }

    if (document.location.href === eltd_root) {
        if (window.history.pushState) {
        } else {
            loadResource("");
        }
        ...
        $j(document).on('click','a[target!="_blank"]:not(.no_ajax):not(.no_link)',function(click){
			...
			if($j(this).closest('.no_animation').length === 0){
                ...
                if(($j(this).attr('href') !== "http://#") && ($j(this).attr('href') !== "#")){
                    ...
                    var start = url.indexOf(eltd_root);
